...
- No behaviour change to the completion flow itself: the buyer's Complete / Report Issue dialog already calls the `complete_order` function, which releases the payout immediately, and the existing auto-complete job still handles the 48h fallback. This work is display only.

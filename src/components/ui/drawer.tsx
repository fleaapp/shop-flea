...
const DrawerContent = React.forwardRef<React.ElementRef<typeof DrawerPrimitive.Content>, React.ComponentPropsWithoutRef<typeof DrawerPrimitive.Content>>(({
  ...
  ...props
    ...
    <DrawerOverlay />
    <DrawerPrimitive.Content ref={ref} className={cn("fixed inset-x-0 bottom-0 z-50 mt-24 flex h-auto max-h-[80svh] flex-col rounded-t-3xl bg-background", className)} {...props}>
      <div className="mx-auto my-3 h-2 w-[80px] rounded-full bg-muted-foreground/30" />
      {children}

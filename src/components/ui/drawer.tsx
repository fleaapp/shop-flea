...
const DrawerContent = React.forwardRef<React.ElementRef<typeof DrawerPrimitive.Content>, React.ComponentPropsWithoutRef<typeof DrawerPrimitive.Content>>(({
  ...
  ...props
    ...
    <DrawerOverlay />
    <DrawerPrimitive.Content ref={ref} onOpenAutoFocus={(e) => e.preventDefault()} className={cn("fixed inset-x-0 bottom-0 z-[60] flex flex-col rounded-t-3xl bg-background outline-none focus:outline-none focus-visible:outline-none", className)} style={{ top: 'calc(env(safe-area-inset-top, 0px) + 24px)' }} {...props}>

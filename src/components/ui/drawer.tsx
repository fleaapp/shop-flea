...
const DrawerOverlay = React.forwardRef<React.ElementRef<typeof DrawerPrimitive.Overlay>, React.ComponentPropsWithoutRef<typeof DrawerPrimitive.Overlay>>(({
  ...
  return <DrawerPrimitive.Overlay ref={innerRef} className={cn("fixed inset-x-0 bottom-0 top-[calc(-1*env(safe-area-inset-top,0px))] z-[60] bg-foreground/50", className)} {...props} />;
});
...
const DrawerContent = React.forwardRef<React.ElementRef<typeof DrawerPrimitive.Content>, React.ComponentPropsWithoutRef<typeof DrawerPrimitive.Content>>(({
  ...
  ...props
    ...
    <DrawerOverlay />
    <DrawerPrimitive.Content ref={ref} onOpenAutoFocus={(e) => e.preventDefault()} className={cn("kb-surface fixed inset-x-0 bottom-0 z-[60] flex flex-col rounded-t-3xl bg-background outline-none focus:outline-none focus-visible:outline-none", className)} style={{ top: 'calc(env(safe-area-inset-top, 0px) + 24px)' }} {...props}>

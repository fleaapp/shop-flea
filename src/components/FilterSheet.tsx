...
// Brand filter sub-component - dropdown style
const BrandFilterSection = ({ selectedBrands, onToggleBrand, onClearBrands }: {
  ...
  const groupedBrands = filteredBrands.reduce<Record<string, typeof filteredBrands>>((acc, brand) => {
    const letter = brand.display_name[0].toUpperCase();
    if (!acc[letter]) acc[letter] = [];
    acc[letter].push(brand);
    return acc;
  }, {});

  const selectedDisplay = selectedBrands.length > 0
    ? brands
        .filter(b => selectedBrands.includes(b.brand_name))
        .map(b => b.display_name)
        .join(', ')
    : '';

  return (
    <div ref={containerRef} className="py-3 border-t border-border">
      ...
      <button
        ...
        className="w-full flex items-center justify-between h-10 px-3 rounded-xl border border-border bg-background text-sm"
      >
        <span className={`truncate ${selectedBrands.length === 0 ? 'text-muted-foreground' : 'text-foreground'}`}>
          {selectedBrands.length === 0 ? 'Select brands...' : selectedDisplay}
        </span>
        ...
        <div className="mt-2 rounded-xl border border-border bg-background shadow-lg overflow-hidden">
          ...
          <div className="relative p-2 border-b border-border">
            ...
            <Input
              ...
              className="pl-9 h-9 rounded-lg bg-muted/50 border-0 text-sm"
              autoFocus
            />
            {brandSearch && (
              <button
          ...
          <div className="max-h-52 overflow-y-auto">
            {loading ? (
              ...
              <div className="px-3 py-3 text-sm text-muted-foreground">No brands found</div>
            ) : (
              Object.keys(groupedBrands).sort().map(letter => (
                <div key={letter}>
                  <div className="px-3 py-1 text-xs font-semibold text-muted-foreground bg-muted/30 sticky top-0">
                    {letter}
                  </div>
    ...
    <Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerContent className="max-h-[90dvh] rounded-t-3xl border-0 bg-background">
                ...
                return (
                  ...
                  >
                    <span
                      ...
                      style={{ background: COLOUR_SWATCHES[colour] || COLOUR_SWATCHES['Multi / Patterned'] }}
                    />
                    {colour}
                  </button>
                );
        ...
        <div className="bg-background border-t border-border">
          ...
          {/* Buttons */}
          <div className="px-6 pt-4 pb-[calc(1rem+env(safe-area-inset-bottom))] flex gap-4">
            <Button
              variant="outline"
              onClick={handleReset}

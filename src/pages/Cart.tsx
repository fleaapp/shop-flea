...
const Cart = () => {
  ...
  const renderOrderCard = (group: OrderGroup, showShadow = false) => {
    ...
    // Fee actually charged at checkout. Never recalculated - historical orders
    // were charged under the fee rules that applied at the time.
    const processingFee = Math.round(group.orders.reduce((sum, o: any) => sum + Number(o.secure_checkout_fee || 0), 0) * 100) / 100;
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className="flex min-w-0 items-baseline text-sm text-foreground">
            ...
            <span className="min-w-0 truncate font-semibold">@{sellerUsername}</span>
                        ...
                        <>
                          {bundleText && (
                            <div className="px-4 py-2 bg-accent/30 text-center text-xs text-accent-foreground">
                              <span className="mr-1">{bundleText.emoji}</span>
                              <span className="font-bold">{bundleText.label}</span>{' '}

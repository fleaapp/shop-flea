    ...
    <div className="fixed inset-0 bg-background pb-24 overflow-hidden flex flex-col" style={{ touchAction: 'pan-x', overscrollBehavior: 'none' }}>
      ...
      <div className="mt-5 max-[430px]:mt-4 max-[393px]:mt-3 max-[375px]:mt-2 flex justify-center items-center gap-2">
        ...
        <div className="flex items-center rounded-full bg-muted p-1 w-48 overflow-hidden">
          <button onClick={() => setActiveTab('listings')} className={`flex min-w-0 items-center justify-center gap-1.5 rounded-full flex-1 px-4 py-2.5 text-sm font-medium transition-all ${activeTab === 'listings' ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground'}`}>
            Listings
            {activeListings.length > 0 && (
              <span className={`flex h-5 w-5 items-center justify-center rounded-full text-[10px] font-semibold ${activeTab === 'listings' ? 'bg-muted text-muted-foreground' : 'bg-card text-foreground/70'}`}>
                {activeListings.length}
              </span>
            )}
          </button>
          <button onClick={() => setActiveTab('sold')} className={`flex min-w-0 items-center justify-center gap-1.5 rounded-full flex-1 px-4 py-2.5 text-sm font-medium transition-all ${activeTab === 'sold' ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground'}`}>
            Sold
            {soldListings.length > 0 && (
              <span className={`flex h-5 w-5 items-center justify-center rounded-full text-[10px] font-semibold ${activeTab === 'sold' ? 'bg-muted text-muted-foreground' : 'bg-card text-foreground/70'}`}>
                {soldListings.length}
              </span>
            )}
          </button>

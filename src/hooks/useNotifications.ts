...
export const getNotificationMessage = (type: string, username?: string, listingTitle?: string | null, rawMessage?: string | null): string => {
  ...
  const soldType =
    ...
    type === 'cart_wishlist_item_sold';
  // Sold alerts are bundle-aware and generated by the backend ("... bundle of 2
  // items has just sold."). Legacy rows stored only the listing title, so only
  // trust the stored copy when it is a full sentence.
  const soldMessageIsAuthoritative = soldType && !!rawMessage && /just sold\./.test(rawMessage);

  const usesAuthoritativeMessage =
    soldMessageIsAuthoritative ||

...
// Check for spaced out words (e.g., "i n s t a")
function checkSpacedPatterns(text: string, patterns: string[]): boolean {
  const stripped = stripAllNonAlphanumeric(text);
  
  for (const pattern of patterns) {
    const strippedPattern = stripAllNonAlphanumeric(pattern);
    if (stripped.includes(strippedPattern)) {
      return true;
    }
  }
  ...
  try {
    ...
    if (profile?.status === 'blocked') {
      return new Response(
        JSON.stringify({
          isBlocked: true,
          reason: 'Your account has been temporarily restricted due to repeated guideline violations. If you believe this is a mistake, contact support.',
          userBlocked: true,
        }),
    ...
    // Fields to check based on content type
    let fieldsToCheck: string[] = [];
    
    if (contentType === 'listing') {
      fieldsToCheck = ['title', 'description', 'brand'];
    } else if (contentType === 'comment') {
      fieldsToCheck = ['content'];
    } else {
      fieldsToCheck = Object.keys(content);
    }
    ...
    for (const field of fieldsToCheck) {
      ...
      if (value && typeof value === 'string') {
        ...
        if (result.isBlocked) {
          console.log(`Content blocked - Field: ${field}, Category: ${result.category}, Text: "${value.substring(0, 100)}..."`);
          return new Response(
            JSON.stringify({
              isBlocked: true,
    ...
    console.log(`Content approved - Type: ${contentType}`);
    return new Response(
